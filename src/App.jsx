import banner_pic from './assets/banner.jpg'
import home_pic_1 from './assets/home1.jpg'
// import home_pic_2 from './assets/home2.jpg'
import home_pic_3 from './assets/home3.jpg'
import home_pic_4 from './assets/home4.jpg'
import home_pic_5 from './assets/home5.jpg'
import home_pic_6 from './assets/home6.jpg'
import home_pic_7 from './assets/home7.jpg'

function App() {
  return (
    <div id='main-content'>
      <div id='banner'>
        <img src={banner_pic} className='w-100'></img>
        <span id='name'>Ruoxi Li</span>
        <nav id='nav-bar' className='d-flex flex-column justify-content-center'>
          <a className='nav-link text-center py-4 py-md-5'>Background</a>
          <a className='nav-link text-center py-4 py-md-5'>Animation</a>
          <a className='nav-link text-center py-4 py-md-5'>Illustration</a>
        </nav>
        <div id='icon-bar' className='d-flex m-3'>
          <a href='mailto:gli@live.ca'>
            <img className='mx-2' width='25' height='25' src="data:image/png;base64," alt="mail"></img>
            <span>gli@live.ca</span>
          </a>
          <a href='' target='_blank'>
            <img className='mx-2' width='25' height='25' src="data:image/png;base64," alt="pencil"></img>
            <span>resume</span>
          </a>
          <span className='flex-grow-1'></span>
          <a href='https://www.instagram.com/m1dnightingale/' target='_blank'>
            <img className='mx-2' width='25' height='25' src="data:image/png;base64," alt="instagram"></img>
          </a>
          <a href='https://www.linkedin.com/in/ruoxi-li-604b5a304/' target='_blank'>
            <img className='mx-2' width='25' height='25' src="data:image/png;base64," alt="linkedin"></img>
          </a>
        </div>
      </div>

      <div className='d-flex border-bottom p-3 pb-md-4 m-3 m-md-4'>
        <div>
          <h1 id='about-me'>About me</h1>
          <span>
            Ruoxi Li is a diligent animation student at Sheridan College.
            She is passionate about telling stories and working with fellow creatives.
          </span>
        </div>
        <img src=''/>
      </div>

      <div className='d-flex mx-3 mx-md-4'>
        <div className='first-column d-flex flex-column'>
          <a>
            <img src={home_pic_1} className='w-100 p-1 p-md-2'/>
          </a>
          <a>
            <img src={home_pic_5} className='w-100 p-1 p-md-2'/>
          </a>
        </div>
        <div className='second-column d-flex flex-column'>
          <a>
            <img src={home_pic_6} className='w-100 p-1 p-md-2'/>
          </a>
          <a>
            <img src={home_pic_6} className='w-100 p-1 p-md-2'/>
          </a>
        </div>
        <div className='third-column d-flex flex-column'>
          <a>
            <img src={home_pic_3} className='w-100 p-1 p-md-2'/>
          </a>
          <a>
            <img src={home_pic_4} className='w-100 p-1 p-md-2'/>
          </a>
          <a>
            <img src={home_pic_7} className='w-100 p-1 p-md-2'/>
          </a>
        </div>
      </div>
    </div>
  )
}

export default App
